  ...
  return (
    <div className="flex flex-col w-96 py-4 border-[1px] border-dark-border bg-accent-bg rounded-md mb-2">
      <div className="flex items-center mb-4 px-4">
        ...
        <Link
          to={`user/${post.authorId.displayName}`}
      ...
      {post.imageUrl && (
        <img
          src={post.imageUrl}
          alt="Post"
          className="max-w-[384px] h-[384px] object-cover border border-dark-border rounded-sm mb-4"
          loading="lazy"
        />
      )}
      <PostActions post={post} />
      <p className="text-xs my-2 px-2">
        <strong>{post.authorId.displayName}</strong> {post.content}
      </p>
    </div>
  );
};
