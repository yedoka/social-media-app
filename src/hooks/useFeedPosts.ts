...
import { addComment, fetchPosts, likePost } from '@/services/api/posts';
...
export const useFeedPosts = () => {
  ...
  const handleAddComment = async ({ postId, text }: { postId: string, text: string }) => {
    await addCommentMutation.mutate({ postId, text });
  };

  return { data, isLoading, error, handleLikePost, handleAddComment }
};